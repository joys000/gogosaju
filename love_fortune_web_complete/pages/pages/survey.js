  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        width: '80%',
        maxWidth: '330px',
        backgroundImage: 'url(/TEXT_BOX.png)',
      ...
      <div style={{
        ...
        {["매우 그렇다", "그렇다", "보통이다", "아니다", "전혀 아니다"].map((opt, i) => (
          <button key={i} onClick={() => handleAnswer(i + 1)}
            style={{
              ...
              backgroundColor: answers[step] === (i + 1) ? '#f8cdd4' : '#fff'
            }}
          >
            {opt}
          </button>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
          <button onClick={handleBack} disabled={step === 0}>이전</button>
          <button onClick={handleNext} disabled={answers[step] == null}>다음</button>
        </div>
        <button onClick={handleRestart} style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: '#666' }}>
          처음으로 돌아가기
        </button>
      </div>
    </div>
